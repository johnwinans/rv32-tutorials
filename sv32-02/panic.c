...
uint32_t panic_stack[1024] __attribute__((used)) __attribute__ ((aligned (16)));
...
void panic_dump(const char *msg)
{
	...
	printf("[%ld] panic: mtime = 0x%16.16llx %54s", mhartid, clint_get_mtime(), trailer);
	//printf("[%ld] panic: mtime = 0x%16.16llx\n", mhartid, clint_get_mtime());
...
/**
* S-mode panic entry.
* Since S-mode can not access all the CSRs, ebreak into M-mode and 
* panic from there.
***************************************************************************/
__attribute__ ((naked)) void stpanic(void)
{
	asm volatile("ebreak");
}
...
/**
* M-mode panic entry.
* Save all the regs and dump the machine state.
***************************************************************************/
__attribute__ ((naked)) void mtpanic(void)
{
	// XXX save and print the mie rego so can disable ASAP!
	...
	asm volatile("la	x2,panic_stack+4096");


	// The gcc docs say that we can ONLY count on asm instructions working in a naked func
	asm volatile("addi	a0,x0,0");			// set the msg arg to null
