...
// This is called in S-mode <---------------- !!!!


void main()
{
	//asm volatile("ebreak");  // do this anywhere to enter M-mode and panic/halt qemu

	DBG("entered\n");
	//hexdump(main, 0x100);

	sv32_setup();
    csr_set_sstatus(SSTATUS_SUM);	// allow S-mode to access U-mode virtual memory
    ...
    csr_clear_sstatus(SSTATUS_SPP);		// U-mode is zero
    //csr_set_sstatus(SSTATUS_SPP_U);	// therefore this is not necessary
